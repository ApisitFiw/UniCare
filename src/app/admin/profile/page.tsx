...
export default function AdminProfilePage() {
  ...
  useEffect(() => {
    ...
    let currentAdminData: Profile | null = null;

    // 1. Try finding in USERS_KEY (synced users list)
    try {
        ...
        if (Array.isArray(parsedUsers)) {
          ...
          if (match?.name) {
            currentAdminData = {
              fullName: match.name,
              email: match.email,
              phone: match.phone || "089-876-5432",
            };
          }
        }
      }
    ...
    // 2. Try ADMIN_PROFILE_KEY if matches current session
    if (!currentAdminData) {
      const saved = window.localStorage.getItem(ADMIN_PROFILE_KEY);
      if (saved) {
        try {
    ...
    // 3. Fallback to ADMIN_ACCOUNTS or session
    if (!currentAdminData) {
      ...
      } else if (session?.name && session.role === "admin") {
        currentAdminData = {
    ...
    if (currentAdminData) {
      setProfile(currentAdminData);
      setDraft(currentAdminData);
      window.localStorage.setItem(
        ADMIN_PROFILE_KEY,
        JSON.stringify(currentAdminData),
      );
    }
  }, []);
  ...
  function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    setDraft(cleaned);
    setIsEditing(false);
    window.localStorage.setItem(ADMIN_PROFILE_KEY, JSON.stringify(cleaned));

    // 1. Update session if currently logged in as admin
    const session = getDemoSession();
    ...
    // 2. Synchronize with System Users list (unicare_demo_system_users)
    try {
      const savedUsers = window.localStorage.getItem(USERS_KEY);
        ...
        systemUsers = systemUsers.map((u: any) => {
          const isTarget =
            ...
            ((u.email && u.email.toLowerCase() === targetEmail) ||
              u.name === targetName ||
              (u.email && u.email.toLowerCase() === cleaned.email.toLowerCase()));
    ...
    // 3. Dispatch events to notify Header, AccountBar, and User Management
    ...
    setAlert({
      ...
      message: "ข้อมูลส่วนตัวผู้ดูแลระบบได้รับการอัปเดตและซิงค์เข้าระบบแล้ว",
    });
  }
  ...
  function handleDeleteAccount() {
    ...
    try {
      ...
      if (!Array.isArray(systemUsers) || systemUsers.length === 0) {
        systemUsers = ADMIN_ACCOUNTS.map((a) => ({ ...a }));
      }
      ...
      <main className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6 lg:p-8">
          ...
          <div className="grid md:grid-cols-[260px_1fr]">
            ...
            <div className="flex flex-col items-center border-b border-slate-100 bg-slate-50/60 px-6 py-8 text-center md:border-b-0 md:border-r">
              ...
              <span className="mt-5 inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-[11px] font-bold text-emerald-700">
                <span className="h-2 w-2 rounded-full bg-emerald-500" /> ผู้ดูแลระบบ (Admin)
              </span>
            </div>
        ...
        <section className="overflow-hidden rounded-2xl border border-rose-200 bg-white shadow-xs">
          <div className="flex items-center gap-3 border-b border-rose-100 bg-rose-50/50 px-5 py-4 sm:px-6">
            ...
            <div>
              <h2 className="text-base font-bold text-rose-900">ลบบัญชีผู้ดูแลระบบ</h2>
          ...
          <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7">
            <div className="max-w-xl">
              ...
              <p className="mt-1 text-xs text-slate-500 leading-relaxed">
                หากคุณลบบัญชี บัญชีของคุณจะถูกปิดการใช้งานทันทีและไม่สามารถเข้าสู่ระบบผู้ดูแลระบบได้
                ทั้งนี้ผู้ดูแลระบบหลักสามารถเข้า <strong className="text-slate-700">กู้คืนบัญชี</strong> ในระบบจัดการบัญชีผู้ใช้ได้ในภายหลัง
              </p>
            </div>
          ...
          >
            ...
            <form onSubmit={changePassword} className="space-y-4 p-6">
              <PasswordField
                ...
                onToggle={() => setVisible({ ...visible, current: !visible.current })}
              />
              <PasswordField
                label="รหัสผ่านใหม่"
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setAlert({
                    type: "success",
                    title: "ส่งคำขอแล้ว",
                    message: "โหมดทดลอง: ระบบจำลองการส่งลิงก์รีเซ็ตรหัสผ่านไปยังอีเมลแล้ว",
                  });
                }}
              ...
              <div className="flex justify-end gap-3 border-t border-slate-100 pt-5">
                ...
                <button
                  type="submit"
                  className="rounded-lg bg-emerald-600 px-5 py-2.5 text-xs font-bold text-white hover:bg-emerald-700 transition cursor-pointer"
                >
                  บันทึกรหัสผ่านใหม่
                </button>
              </div>
        ...
        >
          ...
          >
            ...
            <div className="mt-4 rounded-xl bg-amber-50 p-3.5 border border-amber-200/70 text-xs text-amber-800 leading-relaxed">
              ...
              และคุณจะไม่สามารถล็อกอินเข้าสู่ระบบผู้ดูแลระบบได้อีกจนกว่าจะได้รับการกู้คืนบัญชี
            </div>
      ...
      {alert && (
        ...
        >
          ...
          >
            ...
            <h2 className="mt-5 text-lg font-bold text-slate-800">{alert.title}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-500">{alert.message}</p>
            <button
              type="button"
              onClick={() => setAlert(null)}
              className="mt-6 w-full rounded-lg bg-emerald-600 py-3 text-sm font-bold text-white hover:bg-emerald-700 transition cursor-pointer"
            >
              ตกลง
            </button>
          </div>
...
function ProfileField({
  ...
  return (
    <label className={fullWidth ? "sm:col-span-2" : ""}>
      ...
      <span
        className={`flex items-center gap-3 rounded-xl border px-4 transition ${
          disabled
            ? "border-slate-100 bg-slate-50 text-slate-400"
            : "border-emerald-300 bg-white text-emerald-700 ring-2 ring-emerald-50"
        }`}
      >
        {icon}
        <input
          ...
          className="min-w-0 flex-1 bg-transparent py-3 text-sm text-slate-700 outline-none disabled:cursor-not-allowed"
        />
      </span>
    </label>
  );
...
function PasswordField({
  ...
  return (
    <label>
      <span className="mb-2 block text-xs font-bold text-slate-600">{label}</span>
      <span className="flex items-center gap-3 rounded-xl border border-slate-200 px-4 focus-within:border-emerald-400 focus-within:ring-2 focus-within:ring-emerald-50">
