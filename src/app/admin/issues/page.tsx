...
export default function StatusTrackingPage() {
  ...
  const urlProcessedRef = useRef<{ category?: string; create?: string; issueId?: string }>({})
  ...
  useEffect(() => {
    ...
    if (issueId && issues.length > 0 && urlProcessedRef.current.issueId !== issueId) {
      urlProcessedRef.current.issueId = issueId
      handleSelectIssueFromUrl(issueId)
    } else if ((mode === 'create' || area) && urlProcessedRef.current.create !== `${mode}-${area}`) {
      urlProcessedRef.current.create = `${mode}-${area}`
      handleCreateIssueFromUrl(area || undefined)
    } else if (category && urlProcessedRef.current.category !== category) {
      urlProcessedRef.current.category = category
  ...
  return (
    <div className="min-h-screen flex bg-[#f4f7f5] text-slate-800 antialiased font-['Prompt',sans-serif]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-[#f8faf9] text-slate-500 font-semibold border-b border-slate-200/80">
                  <tr>
                    <th className="py-3 px-4 whitespace-nowrap">รหัสเคส / วันที่</th>
                    <th className="py-3 px-4 whitespace-nowrap">หมวดหมู่ / พื้นที่ (Area)</th>
                    <th className="py-3 px-4">รายละเอียดปัญหา</th>
                    <th className="py-3 px-4 whitespace-nowrap">ผู้รับผิดชอบ (Admin)</th>
                    <th className="py-3 px-4 whitespace-nowrap">สถานะปัจจุบัน</th>
                    <th className="py-3 px-4 text-center whitespace-nowrap">จัดการสถานะ / แชทซักถาม</th>
                  </tr>
                </thead>
                      ...
                      >
                        ...
                        <td className="py-3.5 px-4 whitespace-nowrap">
                          {item.status === 'in_progress' && (
                            <span className="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-full text-[10px] font-semibold inline-flex items-center gap-1.5 whitespace-nowrap shrink-0">
                              <span className="w-1.5 h-1.5 rounded-full bg-blue-600 shrink-0"></span>
                              <span className="whitespace-nowrap select-none">{item.statusLabel}</span>
                            </span>
                          )}
                          {item.status === 'resolved' && (
                            <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-[10px] font-semibold inline-flex items-center gap-1.5 whitespace-nowrap shrink-0">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-600 shrink-0"></span>
                              <span className="whitespace-nowrap select-none">{item.statusLabel}</span>
                            </span>
                          )}
                          {item.status === 'pending' && (
                            <span className="px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-full text-[10px] font-semibold inline-flex items-center gap-1.5 whitespace-nowrap shrink-0">
                        ...
                        <td className="py-3.5 px-4 text-center whitespace-nowrap">
                          <div className="inline-flex items-center justify-center gap-2 shrink-0">
                            ...
                            <button
                              ...
                              className="w-8 h-8 rounded-xl bg-slate-50 text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 border border-slate-200 transition-colors duration-150 flex items-center justify-center relative shrink-0 shadow-xs cursor-pointer group"
                            >
                            ...
                            <button
                              ...
                              className="px-3.5 py-1.5 bg-[#1b5e4a] text-white rounded-xl text-[11px] font-medium hover:bg-[#144737] transition-colors duration-150 shadow-xs inline-flex items-center justify-center gap-1.5 shrink-0 cursor-pointer whitespace-nowrap"
                            >
                              <PenSquare className="w-3 h-3 shrink-0" />
                              <span className="whitespace-nowrap select-none">อัปเดตสถานะ</span>
                            </button>
                          </div>
                        </td>
                      </tr>
      ...
      <CaseClarificationDrawer
        ...
        currentUserId={getDemoSession()?.email || getDemoSession()?.name || 'admin'}
        reporterName={activeChatIssue?.reporterName}
        reporterEmail={activeChatIssue?.reporterEmail}
        assignedAdminName={activeChatIssue?.adminName}
      />
